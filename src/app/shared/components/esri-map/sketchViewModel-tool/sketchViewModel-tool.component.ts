...
@Component({
  selector: 'app-sketchViewModel-tool',
  templateUrl: './sketchViewModel-tool.component.html',
  styleUrls: ['./sketchViewModel-tool.component.scss']
})
export class SketchViewModelToolComponent implements OnInit {
  ...
  async draw(type) {
    ...
    if (!this.sketchViewModel) {
      this.sketchViewModel = new SketchViewModel({
        layer: this.plotDrawGraphicsLayer,
        view: view,
        pointSymbol: {
    ...
    this.sketchViewModel.on("create", (event) => {
      ...
      if (eventInfo && eventInfo.type === "vertex-add") {
        this.vertexAaddPoints.push(eventInfo.added);
      }
      if (eventInfo && eventInfo.type === "cursor-update") {
        this.vertexAUpdatePoints = [eventInfo.coordinates];
        this.coordinates = [];
        const coordinates = [...this.vertexAaddPoints, ...this.vertexAUpdatePoints];
        this.coordinates = this.plotType.THINARROW.handleVertices(coordinates);
        event.graphic.geometry.paths = this.coordinates;
      }
    ...
    switch (type) {
      ...
      case plot.RECTANGLE:
        break;
      case plot.TRIANGLEFLAG:
        break;
      case plot.THINARROW:
        this.sketchViewModel.create(plot.POLYLINE, graphic);
        break;
      case plot.WIDEARROW:
        break;
      case plot.BEZIERLINE:
        break;
      case plot.FREEPOLYGON:
        break;
      case plot.FREEHANDARROW:
        break;
      // case plot.FREELINE:
