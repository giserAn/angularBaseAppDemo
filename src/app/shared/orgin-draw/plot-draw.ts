...
export class PlotDraw {
    ...
    public async drawActivate(type: string, symbol?: any) {
        try {
            ...
            // 每次绘制，如果draw处于激活中...,则重置
            this.removeHanders(this.handers);
            ...
            // 激活绘制工具，并监听
            this.handers.click = this.view.on('click', (evt) => {
                this.isActive = true;

                const point = evt.mapPoint;
                this.addedPoints.push([point.x, point.y, point.z]);
                // this.allPoints = [...this.addedPoints];
                // this.plotType[this.activeType].on.vertexAdd({ vertices: this.allPoints });
            });

            this.handers.move = this.view.on('pointer-move', (evt) => {
                if (this.isActive) {
                    const point = this.view.toMap({ x: evt.x, y: evt.y });
                    if (!point) return;
                    const movePoints = [[point.x, point.y, point.z]];
                    this.allPoints = [...this.addedPoints, ...movePoints];

                    this.plotType[this.activeType].on.cursorUpdate({ vertices: this.allPoints });
                }
            });

            this.handers.dbclick = this.view.on('double-click', (evt) => {
                this.isActive = false;
                const point = this.view.toMap({ x: evt.x, y: evt.y });
                if (!point) return;
                this.addedPoints.push([point.x, point.y, point.z]);
                ...
                this.allPoints = [...this.addedPoints];
                this.plotType[this.activeType].on.drawComplete({ vertices: this.allPoints });
     ...
     * 移除监听事件
     */
    private removeHanders(handers) {
        if (!handers) { return; }
        // tslint:disable-next-line: forin
        for (const key in handers) {
            const hander = handers[key];
            if (hander) { hander.remove(); }
        }
    }
    ...
    */
    ...
    private drawReset() {
        if (!this.plotType[this.activeType]) { return; }
        this.view.cursor = "default";
        if (this.graphicsLayer) { this.graphicsLayer.removeMany(this.plotType[this.activeType].graphics); }
